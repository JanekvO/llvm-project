...
int32_t AMDGPUResourceUsageAnalysis::FunctionResourceInfo::getTotalNumSGPRs(
...
int32_t AMDGPUResourceUsageAnalysis::FunctionResourceInfo::getTotalNumVGPRs(
...
bool AMDGPUResourceUsageAnalysis::runOnMachineFunction(MachineFunction &MF) {
  ...
  // size
  if (AMDGPU::getCodeObjectVersion(*MMI.getModule()) >= AMDGPU::AMDHSA_COV5 ||
  ...
  ResourceInfo = analyzeResourceUsage(MF);

  return false;
}

AMDGPUResourceUsageAnalysis::FunctionResourceInfo
