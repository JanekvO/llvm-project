...
; Negative test: kernel does NOT have the "amdgpu-link-time-lds" attribute,
; but uses an external LDS declaration. The LDS access should still get a
; relocation (IR-driven), and the SHN_AMDGPU_LDS symbol should be emitted.
; However, the kernel descriptor should NOT use a symbolic __amdgpu_lds_size
; reference, and no .amdgpu.callgraph section should be emitted.
